...
class RecipeCategoryResponse(BaseModel):
    ...
    default_net_weight: Optional[float] = None
    created_at: datetime
...
def create_recipe(payload: RecipeInput):
    ...
    try:
        ...
        # 5a. If update_category_default is True and category_id/net_weight are present
        if payload.update_category_default and payload.category_id and payload.net_weight is not None:
             supabase.table("recipe_categories") \
...
def update_recipe(recipe_id: str, payload: RecipeInput):
    ...
    try:
        ...
        # 5a. Update category default if requested
        if payload.update_category_default and payload.category_id and payload.net_weight is not None:
             supabase.table("recipe_categories") \
                .update({"default_net_weight": payload.net_weight, "updated_at": datetime.utcnow().isoformat()}) \
                .eq("id", payload.category_id) \
